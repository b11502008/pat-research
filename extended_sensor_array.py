...
H, W = 75, 75
lam = 28 # 💥 固定 Lambda，抓取幾何細節
J = 20
extend_ratio = 1.0  # 左右各往外延伸 1 倍寬度 (例如 W=128，感測器會從 -128 排到 256)
n_sensors = 128     # 陣列變寬了，感測器數量也要增加 (原本 128 -> 256)
...
plt.tight_layout(); plt.savefig('extended_function_results.png', dpi=300, bbox_inches='tight')
...
print(f"PSNR: {psnr_uncalib:.2f} → {psnr_calib:.2f} dB | SSIM: {ssim_uncalib:.4f} → {ssim_calib:.4f}")
...
# --- 左圖：PSNR ---
bars_psnr = ax1.bar(labels, psnr_values, color=colors, edgecolor='black', width=0.5, linewidth=1.5)
...
ax1.set_ylim(0, max(psnr_values) * 1.3) # 預留空間給上方的數字標籤
...
# --- 右圖：SSIM ---
bars_ssim = ax2.bar(labels, ssim_values, color=colors, edgecolor='black', width=0.5, linewidth=1.5)
...
ax2.set_ylim(0.0, 1.15) # SSIM 滿分是 1.0，最高開到 1.15 留白
...
plt.tight_layout()
plt.savefig('extended_calibration_comparison.png', dpi=300, bbox_inches='tight')
plt.show()